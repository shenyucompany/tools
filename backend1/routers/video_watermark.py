...
def get_watermark_position(position: str, video_width: int, video_height: int, text_width: int, text_height: int):
    """计算水印位置"""
    padding = 20  # 边距
    positions = {
...
@router.post("/api/video/watermark")
async def add_watermark(
    ...
    try:
        ...
        watermark_position = get_watermark_position(position, 0, 0, 0, 0)  # 使用相对位置

        command = [
            'ffmpeg',
            '-i', input_path,
            '-vf', f"drawtext=text='{text}':fontsize={font_size}:fontcolor=white@{opacity_value}:{watermark_position}",
            '-c:v', 'libx264',
            '-preset', 'fast',
            '-c:a', 'copy',
            '-movflags', '+faststart',
        ...
        if process.returncode != 0:
            ...
            raise HTTPException(status_code=500, detail="添加水印失败，请重试")

        # 检查输出文件
        if not os.path.exists(output_path) or os.path.getsize(output_path) == 0:
            raise HTTPException(status_code=500, detail="添加水印失败：输出文件无效")
